...
# configure service docker-compose
for ((i=1; i<=5; i++))
do
  cp -r relay relay$i
  sed -e "s/<router_ip>/$HOST_IP/g" -e "s/<service_name>/relay$i/g" -e "s/<service_id>/'$i'/g" relay/docker-compose.yaml > relay$i/docker-compose.yaml
done
