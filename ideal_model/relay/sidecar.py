...
key_gen = subprocess.Popen(
    ['ndnsec', 'key-gen', Name.to_str(service_name)], stdout=subprocess.PIPE)
cert_install = subprocess.Popen(
    ['ndnsec', 'cert-install', '-'], stdin=key_gen.stdout)
...
subprocess.run(['nlsrc', '-R', '/ndn/test/%C1.Router/router1', '-k',
               'advertise', Name.to_str(service_name)])
print('advertised')
